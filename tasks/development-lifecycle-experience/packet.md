...
当前状态：工作区整理 PR #107 已合并。三项实现和本地验证已完成，改进 PR 为 [#108](https://github.com/InKCre/core-py/pull/108)，尚未合并；远端检查状态由 PR checks 维护。stop 复用现有基于 runtime descriptor 的清理命令；摘要复用已有 append_summary，覆盖空操作、成功和未完成结果；任务规则进入现有 tasks/README.md。
...
下一步：若 #108 的远端检查失败则修正，否则等待 Sir 复核与合并决定。尚无本轮改进 PR 的合并授权。
