...
class Source(SourceBase[SourceConfig], config_cls=SourceConfig):
  ...
  def _parse_email_address(cls, addr_str: Opt[str]) -> Opt[EmailAddress]:
    ...
    if not email_addr:
      return None
    return EmailAddress(email=email_addr, name=cls._decode_header(name) if name else None)
    ...
    try:
        ...
        if full:
          ...
          if last_uid:
            ...
            _, message_numbers = mail.search(
              None,
              "UNSEEN SINCE " + (datetime.now() - timedelta(days=7)).strftime("%d-%b-%Y"),
            )
      ...
      for num in msg_nums:
        ...
        if from_addr and to_addrs:
          ...
          # Update last UID
          state = self.get_state()
          if uid and (not state.get("last_uid") or uid > state.get("last_uid", 0)):
            state["last_uid"] = uid
          ...
          logger.info("Collected email", extra={"uid": email_obj.uid, "subject": subject})
