...
def list_crons(
  ...
  rows, next_cursor = CronManager.list_crons(limit=limit, cursor=cursor)
  return {"crons": rows, "next_cursor": next_cursor}


@ROUTER.get("/{cron_id}")
def get_cron(cron_id: int) -> CronModel:
  result = CronManager.get(cron_id)
  if result is None:
