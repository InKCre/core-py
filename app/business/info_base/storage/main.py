            ...
            if storage_type is None:
                with SessionLocal() as db:
                    storage_type = db.exec(
                        sqlmodel.select(StorageModel.type).where(
                            StorageModel.id == storage_id
                        )
                    ).one()
...
class Storage(abc.ABC, typing.Generic[ConfigTV]):
    ...
    def get_config(self) -> ConfigTV:
        ...
        return typing.cast(
            ConfigTV, self.__configcls__.model_validate(self._storage_config)
        )
    ...
    async def get_content(self, block: BlockModel) -> typing.Any:
        ...
        if self._storage_record is None:
            ...
            raise NotImplementedError(
                "Storage record is not None, should use concrete Storage."
            )
