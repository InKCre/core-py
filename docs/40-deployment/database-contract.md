...
Migrations connect through `MIGRATION_DATABASE_URL`。Application/PostgREST runtimes receive only their login
credentials，不接触 migration owner URL。Passwords are execution inputs and never live in migrations、images or
checked-in profiles。
...
`init` 是 fresh database 的唯一 consumer entry。Commands are repeatable。Catalog reconciliation is import-safe and
does not start FastAPI、scheduler or extensions。
...
Digest 是 consumption boundary；commit tag是 discovery aid，`main` 不可作为 reproducibility pin。Breaking
protocol change必须提升 contract revision，并通过 append-only migration、readiness、generated peer projection 与
portable PostgREST/native acceptance共同证明。
